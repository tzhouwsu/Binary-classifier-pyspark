...
I use simple string search of "dog", "pup", "cat", "kitten", in the comment to identify cat/dog owner.

## step2: build and evaluate classifiers

I use logistic classification (pyspark.ml), with the combined-comments of a user (all his comments) as input-feature, and the owner_tag from step1 as output-label.

I use RegexTokenizer and CountVectorizer to transform the combined-comments to a feature vector
...
I find the largest weight-coefficient in the classifier, and the corresponding keywords. Since I use CountVectorizer, the keywords with the largest coefficient are the most important

## step5: identify creators with cat/dog owners in the audience

I group and count the number_of_users (audience) for each creator, the number_of_dog_owner_users (and fraction), the number_of_cat_owner_users (and fraction).
